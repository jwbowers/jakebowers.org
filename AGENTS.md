...
- Generated pages live in the repo root and are tracked in git (the deploy publishes only tracked files): `index.html`, `publications.html`, `projects.html`, `teaching.html`, `future-politics.html`.
...
- After any data/template/CSS change, re-run `python generate_site.py` and commit the regenerated HTML. The pages must stay tracked: the deploy publishes only git-tracked files, so an untracked page is dropped from the live site. Do not `.gitignore` the generated pages.
